...
class SublayerBlockOutlierAccumulator:
    ...
    def update_arrays(
        ...
        for i, (_, threshold) in enumerate(OUTLIER_SCORE_THRESHOLDS):
            self.outlier_score_gt_counts[i] += int(np.count_nonzero(scores > threshold))
        for i, (_, threshold) in enumerate(MAX_ABS_ERROR_THRESHOLDS):
    ...
    def add_distribution_fields(self, row: dict[str, Any]) -> None:
        ...
        for (suffix, _), count in zip(OUTLIER_SCORE_THRESHOLDS, self.outlier_score_gt_counts):
            row[f"outlier_score_gt_{suffix}_count"] = count
            row[f"outlier_score_gt_{suffix}_pct"] = 100.0 * count / total

        for (suffix, _), count in zip(MAX_ABS_ERROR_THRESHOLDS, self.max_abs_error_gt_counts):
...
def ensure_sublayer_block_outlier_fields(rows: list[dict[str, Any]]) -> None:
    """Ensure generated sublayer block rows have all distribution columns.

    This is intentionally defensive: counts should be generated by
    SublayerBlockOutlierAccumulator.add_distribution_fields(), but explicit
    defaults prevent silent blank CSV cells if a future edit adds a header
    before adding the corresponding row key.
    """
    for row in rows:
        for field in SUBLAYER_BLOCK_OUTLIER_CSV_FIELDS:
            if field not in row:
                row[field] = 0 if field.endswith("_count") else 0.0 if field.endswith("_pct") else ""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if sublayer_block_outliers:
        ensure_sublayer_block_outlier_fields(sublayer_block_outliers)
        write_csv(
    ...
    summary = {
        ...
        "block_outliers": worst_blocks,
        "sublayer_block_outliers": sublayer_block_outliers,
        "skipped": skipped,
